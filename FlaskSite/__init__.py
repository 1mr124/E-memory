...
from FlaskSite.utils.file_utils import (
    writeToFile,
    createNewFolder,
    fileExists,
)  # Import utilities
...
def get_config():
    ...
    env = os.environ.get(
        "FLASK_ENV", "default"
    )  # Default to 'default' if FLASK_ENV is not set Hint: FLASK_ENV=development && flask run
    return config[env]
...
def createApp():
    ...
    )  # restrict it to specific origins React deafult clinet
    ...
    jwt = (
        JWTManager()
    )  # Initialize the JWT manager object, istated of initializing JWTManager twice
